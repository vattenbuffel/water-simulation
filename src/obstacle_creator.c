...
void obstacle_init(float *vertices, float length, float center_x,
                   float center_y) {
    // Init the four corners
    for (int i = 0; i < 4; i++) {
        double alpha = i * 2 * M_PI / 4 + DEG_TO_RAD(45);
        float x = center_x + length / sqrt(2) * cos(alpha);
        float y = center_y + length / sqrt(2) * sin(alpha);
        float z = 0;
...
void obstacles_init(float *vertices, float length, float *center_x,
                    float *center_y, int n_obstacles) {
    for (int i = 0; i < n_obstacles; i++) {
        float *cur_pointer = vertices + OBSTACLES_VERTICES_ARRAY_LENGTH(i);
        obstacle_init(cur_pointer, length, center_x[i], center_y[i]);
    }
}

void obstacle_connecting_vertices(int *connections, int n_obstacles) {
    for (int i = 0; i < n_obstacles; i++) {
        int top_right_corner = 4 * i;
        int bottom_left_corner = top_right_corner + 2;
        for (int j = 0; j < 2; j++) {
            int base_index = i * OBSTACLE_INDEX_ARRAY_LENGTH + j * 3;
